...
class MultipleChoice extends ChoiceQuestion {
     ...
     * @param string $displayType 多选题统计结果展示形式
     * @param array $options 多选题选项
     * @return array $result
     * int $result['code'] 错误码
    ...
    public function update($id, $questionType, $stem, $displayType, $answerType, $options)
    {
        ...
        if ($orm->status == MultipleChoiceStem::STATUS['delete']) {
        ...
        $saveResult = $model->updateMultipleChoice($orm, $this);
        if (!$saveResult) {
            $result['code'] = Resp::SAVE_DATABASE_FAILED;
            return $result;
        }
